...
const getUpdaterFeedUrlByTrack = (track: 'stable' | 'preview' | 'dev'): string => {
  const repoBase = 'https://github.com/taopaox/WeMemo/releases'
...
const getSystemLaunchAtStartup = (): boolean => {
  ...
  try {
    ...
    console.error('[WeMemo] 读取开机自启动状态失败:', error)
    return false
  }
}
...
const syncLaunchAtStartupPreference = () => {
  ...
  if (!result.success && result.error) {
    console.error('[WeMemo] 同步开机自启动设置失败:', result.error)
  }
}
...
function sanitizePathEnv() {
  ...
  if (filtered.length !== parts.length) {
    ...
    console.warn('[WeMemo] 使用白名单裁剪 PATH，移除目录:', removed)
...
const collectLegacySnsCacheMigrationPlan = async (): Promise<SnsCacheMigrationPlan | null> => {
  ...
  const currentBaseDir = configuredCachePath || join(app.getPath('documents'), 'WeMemo')
...
function registerIpcHandlers() {
  ...
  ipcMain.handle('config:clear', async () => {
    if (isLaunchAtStartupSupported() && getSystemLaunchAtStartup()) {
      ...
      if (!result.success && result.error) {
        console.error('[WeMemo] 清空配置时关闭开机自启动失败:', result.error)
      }
    }
    configService?.clear()
  ...
  ipcMain.handle('chat:clearCurrentAccountData', async (_, options?: { clearCache?: boolean; clearExports?: boolean }) => {
    ...
    if (clearCache) {
      ...
      const documentsWeMemoDir = join(app.getPath('documents'), 'WeMemo')
      const userDataCacheDir = join(app.getPath('userData'), 'cache')
      const cacheRootCandidates = [
        configuredCachePath,
        join(documentsWeMemoDir, 'Images'),
        join(documentsWeMemoDir, 'Voices'),
        join(documentsWeMemoDir, 'Emojis'),
        userDataCacheDir
      ].filter(Boolean)
      ...
      for (const wxid of wxidCandidates) {
        ...
        await removePathIfExists(join(documentsWeMemoDir, 'Images', wxid), removedPaths, warnings)
        await removePathIfExists(join(documentsWeMemoDir, 'Voices', wxid), removedPaths, warnings)
        await removePathIfExists(join(documentsWeMemoDir, 'Emojis', wxid), removedPaths, warnings)
    ...
    if (clearExports) {
      ...
      const exportRootCandidates = [
        ...
        join(documentsWeMemoDir, 'exports'),
        join(documentsWeMemoDir, 'Exports')
      ].filter(Boolean)
  ...
  ipcMain.handle(
    ...
    async (
      ...
      const maxConcurrentRaw = Number(process.env.WEMEMO_IMAGE_RESOLVE_BATCH_CONCURRENCY || 3)
  ...
  ipcMain.handle('export:exportSessions', async (event, sessionIds: string[], outputDir: string, options: ExportOptions, controlOptions?: { taskId?: string }) => {
    ...
    const emojiCacheDir = cachePath ? join(cachePath, 'Emojis') : join(app.getPath('documents'), 'WeMemo', 'Emojis')
  ...
  ipcMain.handle('export:exportSession', async (event, sessionId: string, outputPath: string, options: ExportOptions) => {
    ...
    const singleEmojiCacheDir = singleCachePath ? join(singleCachePath, 'Emojis') : join(app.getPath('documents'), 'WeMemo', 'Emojis')
    const workerPath = join(__dirname, 'exportWorker.js')
  ...
  ipcMain.handle('window:openImageViewerWindow', async (_, imagePath: string, liveVideoPath?: string) => {
    ...
    if (imagePath.startsWith('data:')) {
      ...
      const tmpPath = join(app.getPath('temp'), `wememo_preview_${Date.now()}.${ext}`)
