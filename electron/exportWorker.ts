...
if (config.userDataPath) {
  process.env.WEMEMO_USER_DATA_PATH = config.userDataPath
  process.env.WEMEMO_CONFIG_CWD = config.userDataPath
}
process.env.WEMEMO_PROJECT_NAME = process.env.WEMEMO_PROJECT_NAME || 'WeMemo'
...
async function runWeliveEngine() {
  ...
  const rawRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'wememo-welive-raw-'))
