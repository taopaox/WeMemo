...
export class SqlFormatter {
  ...
  public async export(sessionId: any, outputPath: any, options: any, onProgress: any, control: any): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      await writeChunk([
        'BEGIN;',
        'CREATE TABLE IF NOT EXISTS wememo_messages (',
        ...
        '  media_path TEXT',
        ');',
        'CREATE INDEX IF NOT EXISTS idx_wememo_messages_session_time ON wememo_messages (session_id, create_time);',
        ''
      ].join('\n'))
      ...
      for (let i = 0; i < totalMessages; i++) {
        ...
        writeBuffer.push(
          `INSERT INTO wememo_messages (session_id, local_id, message_id, create_time, sender, is_send, local_type, media_type, content, media_path) VALUES (` +
          [
            sqlString(sessionId),
