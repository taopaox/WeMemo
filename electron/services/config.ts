...
const isWorkerThread = process.env.WEMEMO_WORKER === '1'
if (!isWorkerThread) {
  try {
...
export class ConfigService {
  ...
  constructor() {
    ...
    const storeOptions: any = {
      name: 'WeMemo-config',
      defaults,
      projectName: String(process.env.WEMEMO_PROJECT_NAME || 'WeMemo').trim() || 'WeMemo'
    }
    const runningInWorker = process.env.WEMEMO_WORKER === '1'
    if (runningInWorker) {
      const cwd = String(process.env.WEMEMO_CONFIG_CWD || process.env.WEMEMO_USER_DATA_PATH || '').trim()
      if (cwd) {
    ...
    try {
      ...
      if (message.includes('projectName')) {
        const fallbackOptions = {
          ...storeOptions,
          projectName: 'WeMemo',
          cwd: storeOptions.cwd || process.env.WEMEMO_CONFIG_CWD || process.env.WEMEMO_USER_DATA_PATH || process.cwd()
        }
   ...
   * 获取账号目录的真实绝对路径。
   *
   * 这是 WeMemo 统一的账号目录解析入口，所有服务都应通过本方法获取
