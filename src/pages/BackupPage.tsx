...
function BackupPage() {
  ...
  const handleCreateBackup = async () => {
    ...
    try {
      ...
      const defaultPath = `wememo-db-backup-${new Date().toISOString().slice(0, 10)}.${extension}`
      const result = await window.electronAPI.dialog.saveFile({
        title: '保存数据库备份',
        defaultPath,
        filters: [{ name: 'WeMemo 数据库备份', extensions: hasResources ? ['tar'] : ['gz'] }]
      })
  ...
  const handlePickArchive = async () => {
    ...
    try {
      const result = await window.electronAPI.dialog.openFile({
        ...
        properties: ['openFile'],
        filters: [
          { name: 'WeMemo 数据库备份', extensions: ['tar', 'gz', 'tgz'] },
