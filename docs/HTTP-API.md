# WeMemo HTTP API / Push 文档

WeMemo 提供本地 HTTP API（已支持GET 和 POST请求），便于外部脚本或工具读取聊天记录、会话、联系人、群成员和导出的媒体文件；也支持在检测到新消息后通过固定 SSE 地址主动推送消息事件。
...
**状态记忆**：API 服务和主动推送的状态及端口会自动保存，重启 WeMemo 后会自动恢复运行。
...
```json
{
  ...
  "media": {
    "enabled": true,
    "exportPath": "C:\\Users\\Alice\\Documents\\WeMemo\\api-media",
    "count": 1
  },
  "messages": [
    {
    ...
    },
    {
      ...
      "mediaLocalPath": "C:\\Users\\Alice\\Documents\\WeMemo\\api-media\\xxx@chatroom\\images\\abc123.jpg"
    }
  ]
}
```
...
**ChatLab 对接方式**：在 ChatLab 设置中添加远程数据源，`baseUrl` 填 `http://127.0.0.1:5031/api/v1`，Token 填 WeMemo 中配置的 API Token。
